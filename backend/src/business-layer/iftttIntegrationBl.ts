  ...
  private async invokeTrigger(minion: Minion) {
    ...
    try {
      await request({
        ...
        uri: `https://maker.ifttt.com/trigger/when-${minion.minionId}-${
          minion.minionStatus[minion.minionType].status
        }/with/key/${iftttSettings.apiKey}`,
        body: {
          value1: minion.name,
...
export const IftttIntegrationBlSingleton = new IftttIntegrationBl(
  IftttIntergrationDalSingleton,
  MinionsBlSingleton,

...
export abstract class MqttConverterBase {
    ...
    this.mqttClient.on('message', async (topic: string, payload: Buffer) => {
      try {
        ...
        await this.mqttClient.publish(
          `stat/casanet/${parsedMqttMessage.minionId}`,
          JSON.stringify(parsedMqttMessage.minionStatus),
        );
      } catch (error) {
   ...
   */
  public abstract async convertSetStatusToDevice(minionId: string, setStatus: MinionStatus): Promise<MqttMessage>;

  /**
   * Convert casanet get current status message to other device mqtt message format.
   * @param minionId minionId
   */
  public abstract async convertStatusRequestToDevice(minionId: string): Promise<MqttMessage>;
   ...
   */
  public abstract async convertStatusToCasanet(topic: string, data: string): Promise<ParsedMqttMessage>;
}

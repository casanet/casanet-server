...
export abstract class MqttConverterBase {
    ...
    private readonly casanetTopic: string = 'set/casanet/#';
    ...
    public async initClient(brokerIp: string, brokerPort: number): Promise<MqttConverterBase> {
        ...
        this.mqttClient.on('message', async (topic: string, payload: Buffer) => {
            try {
                ...
                /** If message sent from casanet to device, convert it and publish it */
                if (topic.indexOf('set/casanet') !== -1) {
                    ...
                    const mqttMessage = await this.convertToDevice(minionId, status);
                    /** then publish it */
                ...
                const parsedMqttMessage = await this.convertToCasanet(topic, data);
                /** Then publish it */
     ...
     */
    abstract async convertToDevice(minionId: string, setStatus: MinionStatus): Promise<MqttMessage>;
     ...
     */
    abstract async convertToCasanet(topic: string, data: string): Promise<ParsedMqttMessage>;
}

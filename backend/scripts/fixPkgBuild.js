...
(async () => {
	let packagePath = path.join('node_modules', 'es-get-iterator', 'package.json');
	let packageManifest = await fse.readJSON(packagePath);
	packageManifest.main = 'mode.js';
	delete packageManifest.exports;
	fse.writeJSON(packagePath, packageManifest);

	packagePath = path.join('node_modules', 'nanoio', 'package.json');
